* TODO finish generic instanciation of struct types

similar to how GetArrayType and GetEnumSetType use a map to ensure the same type
instance for the same type parameters, there needs to be a map for each struct
type. This does not exist yet and must be done.

* TODO better error message for syntax error in proc def

espectially when the return type is passed in with : instead of ->
the = is missing

* TODO pattern matching for enums, integer, constants, simply for the sdl main loop.
...
technically, my language allows scope local import statements. That is untested.
...
Annotations need to be ignorable. They need to be a type of synatx that isn't seen by regular ast pattern matching.
